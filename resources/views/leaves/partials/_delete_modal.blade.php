{{-- leaves/partials/_delete_modal.blade.php --}}
<div x-show="deleteModalOpen" x-cloak
    ...
    <div @click.outside="deleteModalOpen = false"
        ...
        <div class="flex items-start gap-3 p-5 border-b border-slate-100">
            ...
            <div>
                <h3 class="text-sm font-semibold text-slate-900">Delete this leave?</h3>
                <p class="mt-0.5 text-xs text-slate-500">
                    <span x-text="deleteTarget?.name"></span> · <span x-text="deleteTarget?.dates"></span>
                </p>
            </div>
        </div>
        ...
        <div class="p-5">
            <div class="rounded-lg bg-rose-50 border border-rose-100 divide-y divide-rose-100">
                <p class="px-4 py-3 text-xs text-rose-800">These days go back into the employee's leave balance.</p>
                <p class="px-4 py-3 text-xs text-rose-800">The leave pay is removed once the week is refreshed on the weekly payroll page.</p>
                <p class="px-4 py-3 text-xs text-rose-800">Weeks that are already finalised are protected and will block this.</p>
            </div>
        </div>

        <form :action="deleteTarget ? '{{ url('leaves') }}/' + deleteTarget.id : '#'" method="POST">
            @csrf
            @method('DELETE')
