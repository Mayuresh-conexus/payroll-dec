{{-- leaves/partials/_table.blade.php — recorded leave with edit/delete actions --}}
    ...
    <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
            @forelse ($leaves as $leave)
                @php
                    $days = $leave->calendarDays();
                    $runs = $affected[$leave->id] ?? collect();
                    $isPast = $leave->end_date->isPast();
                @endphp
                <tr class="hover:bg-slate-50/80 {{ $isPast ? 'text-slate-400' : '' }}">
                    <td class="px-4 py-3">
                        <div class="font-medium {{ $isPast ? 'text-slate-500' : 'text-slate-800' }}">
                            {{ $leave->employee?->name ?? '—' }}
                        </div>
                        <div class="font-mono text-[11px] text-slate-400">{{ $leave->employee?->employee_code }}</div>
                        @if ($leave->hours_per_day)
                            <span class="mt-1 inline-flex px-1.5 py-0.5 rounded bg-sky-50 text-sky-700 text-[10px] font-semibold"
                                  title="Hours paid for each day of this leave.">
                                {{ (float) $leave->hours_per_day }} hrs/day
                            </span>
                        @endif
                    </td>
                    <td class="px-4 py-3 text-slate-600">
                        ...
                        @if ($days > 1)
                            <span class="text-slate-400">→</span> {{ $leave->end_date->format('d M Y') }}
                        @endif
                        @if (! $isPast && $leave->start_date->isFuture())
                            <span class="ml-1 inline-flex px-2 py-0.5 rounded-full bg-sky-50 text-sky-700 text-[10px] font-semibold">Upcoming</span>
                        @endif
                    </td>
                    <td class="px-4 py-3 text-center text-slate-600" title="Calendar days in the range. Non-working days are not deducted.">{{ $days }}</td>
                    <td class="px-4 py-3 text-slate-500 text-xs">{{ $leave->reason ?: '—' }}</td>
                    <td class="px-4 py-3">
                        @forelse ($runs as $run)
                            <span class="inline-flex items-center gap-1 mr-1 px-2 py-0.5 rounded-full text-[10px] font-semibold
                                ...
                                title="{{ $run->status === 'final' ? 'Finalised — this leave can no longer be changed' : 'Draft — refresh this week to apply changes' }}">
                                wk {{ $run->week_number }}
                    ...
                    <td class="px-4 py-3 text-slate-500 text-xs">{{ $leave->creator?->name ?? '—' }}</td>
                    <td class="px-4 py-3 text-right">
                        <div class="flex justify-end items-center gap-2 text-slate-500">
                            ...
                            <button type="button" @click="openDeleteModal(@js([
                                'id' => $leave->id,
                                'name' => $leave->employee?->name,
                                'dates' => $leave->start_date->format('d M Y').($days > 1 ? ' → '.$leave->end_date->format('d M Y') : ''),
                            ]))"
                                data-tooltip="Delete"
            ...
            @empty
                <tr>
                    <td colspan="7" class="px-4 py-6 text-center text-sm text-slate-500">
                        No leave recorded yet. Use "Record Leave" to add some.
                    </td>
                </tr>
            @endforelse
        </tbody>
